...
**Honest disclosure.** A 6th attack — `selective_submit` — *does* beat no_op (0.74 on easy, 0.51 on hard) by submitting each claim after a single trivial `coding_engine` call. The root cause is that identity fields are pre-populated correctly in the dirty state, so a minimal-effort agent earns full `final_correctness` credit without doing the real work. We documented this as a v1-grader limitation rather than fixing it last-minute (a fix would invalidate the existing baselines and trained models). The v2 grader fix is to corrupt 2-3 identity fields in `_build_dirty_from_ground_truth` so the agent must repair them. The attack ships in `test_exploits.py` and is segregated from the gate as a "documented limitation" — surfaced for transparency, not hidden.
