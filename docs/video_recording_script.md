...
5. **Test the demo runner once silently** to make sure it runs:
   ...
   ```
   Confirm: drift fires at step 23, final composite score 0.762. If anything else, ping me before recording.
...
| 0:50 | Final episode summary appears | "Final composite score: 0.762." |
| 0:55 | Grader breakdown visible | "That 0.762 is not recovery success. It is the cost of carrying a stale policy model into submit. Closing that gap is what our training pipeline is designed to target." |
...
- **You hit a final score that is not 0.762.** Stop. Run `--seed 44` again. If it still differs, paste the output back to chat — something has changed since today's commits.
